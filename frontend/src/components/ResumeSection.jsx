...
export const CertificationInfo = ({ title, issuer, year }) => (
  <div className={styles.certContainer}>
    {/* Title on the first line */}
    <h3 className={styles.certTitle}>{title}</h3>
    
    {/* Issuer and Year on the second line */}
...
export const EducationInfo = ({ degree, institution, duration, percentage }) => (
  <div className={styles.eduContainer}>
    <div className="flex justify-between items-start">
      ...
      {/* Conditionally render the percentage only if it exists */}
      {percentage && (
        <p className={styles.eduPercentage}>{percentage}</p>
      )}
    </div>
      </div>
);
