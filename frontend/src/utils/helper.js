...
import { format } from "date-fns"; // Moved to the top with other imports
...
export function formatYearMonth(yearMonth) {
  if (!yearMonth || yearMonth.toLowerCase() === "present") {
    return "Present";
  }
  const date = new Date(yearMonth);
  // Check for invalid dates to prevent crashes
  if (isNaN(date.getTime())) {
    return ""; // Or some other fallback text
  }
...
/**
 * Clone a DOM element and run html2canvas on that clone.
...
export const dataURLtoFile = (dataUrl, fileName) => {
  ...
  if (!dataUrl || !dataUrl.includes(',')) {
    console.error("Invalid dataURL received, cannot convert to file.", dataUrl);
  ...
  // This loop is slightly more performant for large files
  while (n--) {
